...
class StoreController extends Controller
{
    ...
    */
    ...
    public function list_products()
    {
        ...
        $max_stock=ProductoModel::orderBy("p_stock",'desc')->get()->first();

        $max_ventas=VentaModel::where("v_estado","=",1)
        ->select(DB::raw("id_producto, SUM(v_cantidad) AS total"))->with('producto')
        ->groupBy('id_producto')->orderBy("total",'desc')->get()->first();
        return view('producto.list',compact('datos','max_stock','max_ventas'));
    }
    ...
    {
        $datos= VentaModel::select('*')->with('producto')->get();
        $max_stock=ProductoModel::orderBy("p_stock",'desc')->get()->first();
        $max_ventas=VentaModel::where("v_estado","=",1)
        ->select(DB::raw("id_producto, SUM(v_cantidad) AS total"))->with('producto')
        ->groupBy('id_producto')->orderBy("total",'desc')->get()->first();

        return view('venta.list',compact('datos','max_stock','max_ventas'));
    }
